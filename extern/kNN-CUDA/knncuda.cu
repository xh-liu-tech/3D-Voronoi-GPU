 ...
 * @param dist         array containing the query_width x ref_width computed distances
 */
__global__ void compute_distance_texture(const cudaTextureObject_t ref,
                                         const int                 ref_width,
                                         const float *             query,
                                         const int                 query_width,
                                         const int                 query_pitch,
 ...
 */
__global__ void compute_sqrt(float * dist, const int width, const int pitch, const int k){
    unsigned int xIndex = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int yIndex = blockIdx.y * blockDim.y + threadIdx.y;
 ...
 */
__global__ void compute_squared_norm(const float * array, const int width, const int pitch, const int height, float * norm){
    unsigned int xIndex = blockIdx.x * blockDim.x + threadIdx.x;
    if (xIndex<width){
        float sum = 0.f;
        for (int i=0; i<height; i++){
 ...
 */
__global__ void add_reference_points_norm(float * array, const int width, const int pitch, const int height, const float * norm){
 ...
 */
__global__ void add_query_points_norm_and_sqrt(float * array, const int width, const int pitch, const int k, const float * norm){
    unsigned int xIndex = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int yIndex = blockIdx.y * blockDim.y + threadIdx.y;
    if (xIndex<width && yIndex<k)
...
bool knn_cuda_global_dev(
    const float* ref_dev,
    const int ref_nb,
    const size_t ref_pitch,
    const float* query_dev,
    const int query_nb,
    const size_t query_pitch,
    const int dim,
    const int k,
    int* knn_index_dev,
...
)
{
    ...
    err0 = cudaMallocPitch((void**)& dist_dev, &dist_pitch_in_bytes, query_nb * size_of_float, ref_nb);
    if (err0 != cudaSuccess) {
    ...
    modified_insertion_sort << <grid1, block1 >> > (dist_dev, dist_pitch, knn_index_dev, index_pitch, query_nb, ref_nb, k);
    if (cudaGetLastError() != cudaSuccess) {
        printf("ERROR: Unable to execute kernel\n");
        cudaFree(dist_dev);
        return false;
...
bool knn_cuda_texture(const float * ref,
    ...
    cudaArray* ref_array_dev = nullptr;
    ...
    err0 = cudaCreateTextureObject(&ref_tex_dev, &res_desc, &tex_desc, nullptr);
    if (err0 != cudaSuccess) {
...
bool knn_cublas(const float * ref,
    ...
    float * dist_dev       = nullptr;
    int   * index_dev      = nullptr;
    float * ref_norm_dev   = nullptr;
    float * query_norm_dev = nullptr;
    size_t  ref_pitch_in_bytes;
    size_t  query_pitch_in_bytes;
    size_t  dist_pitch_in_bytes;
    size_t  index_pitch_in_bytes;
